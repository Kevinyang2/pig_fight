...
def parse_arguments() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description=(
			"使用 YOLO + 跟踪生成 ReID 數據集：按 track ID 分身份，並可劃分 train/val/test。"
		)
	)
	...
	parser.add_argument("--tracker", type=str, default="ultralytics/cfg/trackers/bytetrack.yaml", help="跟踪器配置 YAML")
	...
	parser.add_argument("--format", choices=["folders"], default="folders", help="輸出結構格式。默認為 folders：train/val/test 下按身份分文件夾")
...
def collect_and_save(
	...
	pad: int,
	camera_id: int,
	classes_filter: Optional[Iterable[int]] = None,
	max_per_id: int = 0,
	per_id_counts: Optional[Dict[int, int]] = None,
) -> int:
...
def main() -> None:
	...
	print(
		f"Identities split -> train: {train_n}, val: {val_n}, test: {test_n}. Output: {dataset_root}"
	)
