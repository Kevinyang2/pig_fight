...
class ActivationsAndGradients:
    ...
    def post_process(self, result):
        ...
        logits_ = result[:, 4:]
        boxes_ = result[:, :4]
        _sorted_scores, indices = torch.sort(logits_.max(1)[0], descending=True)
...
class yolov10_heatmap:
    def __init__(
        self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_result, renormalize, img_size
    ):
...
if __name__ == "__main__":
    ...
    # 示例：单张图片
    model(
        r"D:\googleDownload\pig_fight_DEC\valid\images\000001_jpg.rf.2dcec7980067e47995dd7abfe1ce04ef.jpg",
        "result-heatmap-AP-ATFL",
    )
    # 或者：目录
