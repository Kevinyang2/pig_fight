...
class DetectionWindow(QMainWindow):
    ...
    def _apply_global_style(self) -> None:
        ...
        self.setStyleSheet(
            ...
            QPushButton:hover {{
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 rgba(59, 130, 246, 0.3), stop:1 rgba(37, 99, 235, 0.2));
                border: 2px solid rgba(96, 165, 250, 0.5);
                box-shadow: 0 0 20px rgba(96, 165, 250, 0.2);
            }}
            QPushButton:pressed {{
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 rgba(30, 58, 138, 0.5), stop:1 rgba(23, 37, 84, 0.4));
    ...
    def _build_side_panel(self) -> QFrame:
        ...
        layout.setContentsMargins(28, 50, 28, 32)
        layout.setSpacing(8)

        title = QLabel("基于多尺度感知的猪只计数系统")
        ...
        self.model_path_edit.setPlaceholderText("模型权重路径，例如 pig_counts_best.pt")
        self.model_path_edit.setReadOnly(True)
        self.model_path_edit.setFixedHeight(42)

        self.btn_choose_model = QPushButton("选择模型权重")
        self.btn_choose_model.clicked.connect(self._on_choose_model)
        self.btn_choose_model.setMinimumHeight(44)

        self.btn_use_default = QPushButton("使用默认模型 (pig_counts_best.pt)")
        self.btn_use_default.clicked.connect(self._on_use_default_model)
        self.btn_use_default.setMinimumHeight(44)

        self.confidence_label = QLabel("置信度阈值：0.50")
        self.confidence_label.setStyleSheet("color: #cbd5e1; font-size: 13px;")
        ...
        self.btn_image.clicked.connect(self._on_select_image)
        self.btn_image.setMinimumHeight(44)

        self.btn_batch_images = QPushButton("批量图片检测")
        self.btn_batch_images.clicked.connect(self._on_batch_images)
        self.btn_batch_images.setMinimumHeight(44)
        self.btn_batch_images.setStyleSheet("""
            QPushButton {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
        ...
        self.btn_video = QPushButton("选择视频流")
        self.btn_video.clicked.connect(self._on_select_video)
        self.btn_video.setMinimumHeight(44)

        self.btn_webcam = QPushButton("启用本地摄像头")
        self.btn_webcam.clicked.connect(self._on_toggle_webcam)
        self.btn_webcam.setMinimumHeight(44)

        # 网络摄像头输入框
        self.network_cam_label = QLabel("网络摄像头URL")
        self.network_cam_label.setStyleSheet("color: #cbd5e1; font-size: 13px;")
        
        self.network_cam_edit = QLineEdit()
        self.network_cam_edit.setPlaceholderText("例如: rtsp://192.168.1.100:554/stream")
        self.network_cam_edit.setFixedHeight(42)
        
        self.btn_network_cam = QPushButton("连接网络摄像头")
        self.btn_network_cam.clicked.connect(self._on_connect_network_cam)
        self.btn_network_cam.setMinimumHeight(44)
        ...
        self.status_label.setStyleSheet("color: #60a5fa; font-size: 13px; font-weight: 500;")
        ...
        layout.addWidget(self.btn_use_default)
        layout.addSpacing(14)
        layout.addWidget(self.confidence_label)
        layout.addWidget(self.conf_slider)
        layout.addSpacing(14)
        layout.addWidget(self.btn_image)
        layout.addWidget(self.btn_batch_images)
        layout.addWidget(self.btn_video)
        layout.addWidget(self.btn_webcam)
        layout.addSpacing(10)
        layout.addWidget(self.network_cam_label)
        layout.addWidget(self.network_cam_edit)
        layout.addWidget(self.btn_network_cam)
        layout.addSpacing(10)
    ...
    def _build_preview_panel(self) -> QFrame:
        ...
        header.setStyleSheet("""
            color: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                stop:0 #f0f9ff, stop:0.5 #bae6fd, stop:1 #93c5fd);
            font-size: 25px;
            font-weight: 800;
        ...
        json_title.setStyleSheet("""
            color: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                stop:0 #e0f2fe, stop:0.5 #bae6fd, stop:1 #7dd3fc);
            font-size: 20px;
            font-weight: 800;
        """)
        ...
        info_row.setStyleSheet("color: #93c5fd; font-size: 13px;")
    ...
    def _on_confidence_changed(self, value: int) -> None:
        self.confidence_label.setText(f"置信度阈值：{value / 100:.2f}")
    ...
    def _on_connect_network_cam(self) -> None:
        """连接网络摄像头"""
        if not self._ensure_model_loaded():
            return
        
        url = self.network_cam_edit.text().strip()
        if not url:
            QMessageBox.warning(self, "输入错误", "请输入网络摄像头URL")
            return
    ...
    def _load_model(self, weight_path: Path) -> None:
        try:
            ...
            self._append_log(f"✅ 模型加载成功 ({elapsed * 1000:.1f} ms)")
        except Exception as err:
    ...
    def _process_batch_images(self, image_paths: list[Path]) -> None:
        ...
        self._append_log(f"📦 开始批量检测：{total} 张图片")
        ...
        try:
            ...
            self._append_log(f"💡 点击\"保存结果\"可保存所有检测图片和JSON数据")
    ...
    def _update_stream_frame(self) -> None:
        ...
        try:
            assert self.model is not None
            results = self.model.predict(source=frame, conf=conf, verbose=False, stream=False)
            if results:
                annotated, summary, detection_data = self._render_result(frame, results[0])
...
def main() -> None:
    ...
    if login_window.exec_() == QDialog.Accepted:
        ...
        # 可以通过信号获取登录用户名
        login_window.login_success.connect(lambda username: window.setWindowTitle(f"基于多尺度感知的猪只计数系统 - 欢迎 {username}"))
        window.show()
        sys.exit(app.exec_())
    else:
        # 用户取消登录，直接退出
