...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tracker", type=str, default="botsort", help="跟踪器：bytetrack 或 botsort（或自定义yaml路径）"
    )
    ...
    parser.add_argument(
        "--no-fight-crop", dest="fight_crop", action="store_false", help="关闭裁剪，使用整帧打架检测+IoU 融合"
    )
...
def fuse_fight_labels(
    track_ids: np.ndarray, track_boxes: np.ndarray, fight_boxes: np.ndarray, iou_thr: float
) -> dict[int, bool]:
...
class EventManager:
    def __init__(self, smooth_window: int, merge_gap: int):
        ...
        self.active_start: dict[int, int | None] = {}
