...
class BaseFormer(nn.Module):
    ...
    def folding(self, Patches: Tensor, OutputSize: tuple[int, int]) -> Tensor:
        ...
        # ONNX 友好路径：当 stride == kernel 且无重叠时，用纯张量重排替代 F.fold
        if torch.onnx.is_in_onnx_export():
            H, W = OutputSize
            Ph, Pw = self.HPatch, self.WPatch
            nH = H // Ph
            nW = W // Pw
            # 还原到 [B, C, Ph, Pw, nH, nW] -> [B, C, nH, Ph, nW, Pw] -> [B, C, H, W]
            y = Patches.view(B, C, Ph * Pw, nH, nW)
            y = y.view(B, C, Ph, Pw, nH, nW).permute(0, 1, 4, 2, 5, 3).contiguous()
            y = y.view(B, C, H, W)
            return y
