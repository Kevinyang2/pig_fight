...
def find_latest_weights(default_runs_dir: str = 'runs/train') -> Optional[str]:
...
def resolve_test_source(data_yaml_path: Path) -> Union[str, List[str]]:
    ...
    def join_maybe(p: Union[str, Path]) -> Path:
        q = Path(p)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--source', type=str, default=None, help='直接指定推理源(文件/目录/通配符)，覆盖 YAML 中的 test')
    parser.add_argument('--mode', type=str, default='predict', choices=['predict', 'val'], help='运行模式：predict 或 val(评估)')
    parser.add_argument('--split', type=str, default='test', choices=['train', 'val', 'valid', 'test'], help='评估数据划分，val/valid/test/train')
...
def _has_media_files(directory: Path) -> bool:
    if not directory.exists() or not directory.is_dir():
        return False
    exts = {
        'bmp', 'tiff', 'webp', 'pfm', 'png', 'heic', 'jpeg', 'tif', 'mpo', 'dng', 'jpg',
        'm4v', 'mov', 'mpeg', 'mpg', 'mkv', 'ts', 'avi', 'webm', 'gif', 'mp4', 'wmv', 'asf',
    }
    try:
